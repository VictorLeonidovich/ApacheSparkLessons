package _13_Big_Data_Big_Exercise._11_Walkthrough_Step_9_adding_titles_and_using_the_Big_Data_file;
...
public class ViewingFigures {
    public static void main(final String[] args) {
        ...
        try (final JavaSparkContext javaSparkContext = new JavaSparkContext(sparkConf)) {
            ...
            final JavaPairRDD<Integer, Long> step5 = reduced.mapToPair(row -> new Tuple2<>(row._1._2, row._2));

            //Step 6 - add in the total chapter count
            final JavaPairRDD<Integer, Tuple2<Long, Integer>> step6 = step5.join(chapterCountRDD);

            //Step 7 - convert to percentage
            final JavaPairRDD<Integer, Double> step7 = step6.mapValues(value -> (double) value._1 / value._2);

            //Step 8 - convert to scores
            final JavaPairRDD<Integer, Long> step8 = step7.mapValues(value -> {
                if (value > 0.9) {
                    return 10L;
                }
            ...
            //Step 9
            final JavaPairRDD<Integer, Long> step9 = step8.reduceByKey((value1, value2) -> value1 + value2);

            //Step 10
            final JavaPairRDD<Integer, Tuple2<Long, String>> step10 = step9.join(titlesData);
            final JavaPairRDD<Long, String> step11 = step10.mapToPair(row -> new Tuple2<>(row._2._1, row._2._2));

            step11.sortByKey(false).collect().forEach(System.out::println);
            //step10.foreach(toPrint -> System.out.println(toPrint));
        }
